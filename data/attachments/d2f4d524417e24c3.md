...
> 73 |         const responsebody = await response.json();
     |                              ^ SyntaxError: Unexpected end of JSON input
  74 |         console.log('responsebody',responsebody);
  75 |         expect(response.status()).toBe(200);
  76 |         const storeId = response.map(item=>item.locationId);
  77 |         console.log(storeId);
  78 | 
  79 |     })
  80 | })
  81 | 
